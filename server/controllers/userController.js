...
export async function signUpPost(req, res, next) {
  ...
  try {
    ...
    const sentEmail = await Mailer.sendTemplate('confirm-email', mergeObj);
...
export function accountPut(req, res, next) {
  const user = new UserAccounts({ id: req.user.id });
  ...
  user.fetch({ withRelated: 'profile' }).then((user) => {
    if ('password' in req.body) {
...
export async function forgotPost(req, res, next) {
  ...
  try {
    ...
    const sentEmail = await Mailer.sendTemplate('forgot-password', { name: firstName, email, url: resetUrl });
                    ...
                    .then((user) => {
                      ...
                      new UserAccounts({ email: profile.email })
                            ...
                            .then((user) => {
                              ...
                              user.set('facebook', profile.id);
                              user.save().then(user => res.send({ token: generateToken(user), user }));
                            });
                    });
      }
    });
