...
function createUserAccounts(amountToCreate) {
  amountToCreate -= 1; // to account for the admin user
  ...
  for (let i = 0; i < amountToCreate; i++) {
    const randomNumber = faker.random.number(1);
    const gender = randomNumber; // 0 is male, 1 is female
...
async function seedUsers(amountToCreate) {
  ...
  const users = createUserAccounts(amountToCreate);

  for (const user of users) {
    try {
      const userId = await knex(accountsTable).insert(user.userAccount, 'id');
      const profileObj = Object.assign({}, { user_account_id: userId[0] }, user.userProfile);
  ...
  const userAccounts = await knex.select().table(accountsTable);
