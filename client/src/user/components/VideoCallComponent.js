...
class VideoCallComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    this.timerID = setInterval(() => this.shouldRoomOpen(conversationTimeUTC, openXMinsBefore), 60000); // check every minute if the room should open
        ...
        {this.state.roomOpen ?
          (
            <Embed
              ...
              url={`https://appear.in/${conversationId}`}
              active
            />
          ) : (
